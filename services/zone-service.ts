...
class ZoneService {
    ...
    async sterilizeCat(zoneID: string | string[], catID: string | string[], newCat: SterilizedCat): Promise<SterilizedCat> {
        let interestZone = await this.findById(zoneID);
    ...
    private async validateZoneEditting(body: InterestZone): Promise<void> {
        ...
        if (body.status === Status.DONE && body.noUnsterilizedCats > 0) throw new ZoneValidationError(400, ZoneError.STATUS);
